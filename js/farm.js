/* ═══════════════════════════════════════════════════════════════
   KRISHI DRISHTI — 3D Farm Scene v4
   Key fixes:
   · PlaneGeometry bumps use setY (not setZ) — correct axis
   · Crops are 1.8–2.8 units tall — clearly visible
...
const FarmScene = (() => {
  ...
  /* phi=0.82 → ~35° elevation so crops are clearly in view */
  ...
  function init() {
    ...
    /* SKY colours — THREE guaranteed loaded here */
    SKY = {
  ...
  /* ══════════════════════════════════════════════════════════
     GROUND
     PlaneGeometry is in XY-plane → rotation.x=-PI/2 maps Y→vertical.
  ...
  /* ══════════════════════════════════════════════════════════
     CROPS  ← THE KEY FIX: much taller, simpler geometry, 
     clearly visible from any camera angle
  ══════════════════════════════════════════════════════════ */
  function _crops() {
    const ROWS = 7, COLS = 11;
    /* Centre the grid — COLS × dx should be ≤ 22 */
  ...
  function _makeCrop(x, z) {
    ...
    /* Height: 1.8 – 2.6 units (much taller → easy to see) */
    ...
    /* ── LEAVES: 3 large crossing planes ─────────────────── */
    ...
    for (let i = 0; i < 3; i++) {
      ...
      /* Wide drooping leaf — 1.2 wide, 0.35 tall */
  ...
  function _sensorNodes() {
    [[-6, 9], [2, 9], [10, 9], [-6, -9], [2, -9], [10, -9]].forEach((pos, i) => {
      ...
      /* glow orb — own material per node */
  ...
  const BADGE = {
    ...
    drought: { icon:'☀️',  label:'Drought Stress',        sub:'XGBoost · Soil moisture drop', bg:'#e65100' },
